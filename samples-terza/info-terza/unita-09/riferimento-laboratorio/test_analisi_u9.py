"""Suite del LAB-PY-U9 - versione di riferimento, casi pubblici L01-L12 completa.

I casi L sono locali al laboratorio e non sono nuovi codici del catalogo PY.
La suite non si affida all'ordinamento delle durate misurate: le prove del
timer controllano il protocollo e la forma dei dati, non la velocità
relativa dei due algoritmi. I casi L05-L07 verificano i contatori C, D e S
delle funzioni complete fornite per il confronto di Fibonacci.

Il caso L12 è il caso proprio progettato dal riferimento: sostituirlo con un
caso motivato dal proprio studio quando lo si compila nella propria copia.
"""
...
# --- L12: caso progettato dal riferimento -----------------------------------


def test_l12_caso_dello_studente():
    """Caso proprio del riferimento: quante chiamate esegue davvero un lotto.

    Cosa discrimina: il numero totale di chiamate alla funzione misurata deve
    essere `ripetizioni * chiamate_per_lotto`, perché ogni ripetizione esegue
    un lotto intero. Conclusione impropria che rileva: un misuratore che
    esegue `ripetizioni` chiamate in tutto, oppure che divide la durata per
    un numero di chiamate diverso da quello eseguito, produce campioni
    numerici plausibili ma riferiti a un protocollo diverso dalla consegna.
    """
    chiamate = []

    def funzione_contata(valori):
        chiamate.append(1)
        return riepilogo_due_scansioni(valori)

    valori = [1, -2, 3]
    campioni = misure.misura_scansione(funzione_contata, valori, 5, 3)
    assert len(campioni) == 5
